            ...
            <div className="py-8 px-12 mx-auto max-w-screen-md border rounded-2xl shadow-inner">
                ...
                <form onSubmit={sendEmail}  className="space-y-8">
                    <div>
                        ...
                        <input type="text" name="user_name" id="user_name" className="shadow-inner  border border-zinc-200 text-zinc-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-zinc-800 dark:border-zinc-700 dark:placeholder-zinc-400 dark:text-white dark:focus:ring-primary-500 dark:shadow-sm-light" placeholder="John Smith" required />
                    </div>
                    <div>
                        <label htmlFor="user_email" className="block mb-2 text-sm font-medium text-zinc-900 dark:text-zinc-300">Email</label>
                        <input type="email" name="user_email" id="user_email" className="shadow-inner border border-zinc-200 text-zinc-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-zinc-800 dark:border-zinc-700 dark:placeholder-zinc-400 dark:text-white dark:focus:ring-primary-500 dark:shadow-sm-light" placeholder="name@email.com" required />
                    </div>
                    <div>
                        <label htmlFor="subject" className="block mb-2 text-sm font-medium text-zinc-900 dark:text-zinc-300">Subject</label>
                        <input type="text" name="subject" id="subject" className="block p-3 w-full text-sm text-zinc-900 rounded-lg border border-zinc-300 shadow-inner focus:ring-primary-500 focus:border-primary-500 dark:bg-zinc-800 dark:border-zinc-700 dark:placeholder-zinc-400 dark:text-white dark:focus:ring-primary-500 dark:shadow-sm-light" placeholder="What is this about" required />
                    </div>
                    <div className="sm:col-span-2">
                        <label htmlFor="message" className="block mb-2 text-sm font-medium text-zinc-900 dark:text-zinc-300">Message</label>
                        <textarea id="message" name="message" rows="6" className="block p-2.5 w-full text-sm text-zinc-900 rounded-lg shadow-inner border border-zinc-300 focus:ring-primary-500 dark:bg-zinc-800 dark:border-zinc-700 dark:placeholder-zinc-400 dark:text-white dark:focus:border-primary-500" placeholder="No SQL Injections please..."></textarea>
                    </div>
                    <Button disabled={submitted} type="submit" className={`py-3 px-5 text-sm font-medium text-center text-white rounded-lg ${submitted? "bg-zinc-900 dark:bg-zinc-600": "bg-zinc-800 hover:bg-zinc-900 sm:w-fit focus:outline-none dark:bg-zinc-700 dark:hover:bg-zinc-800"} `}>
                        <IoSend />
                        <p>Send Message</p>
                        </Button>
                </form>
